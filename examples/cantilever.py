...
class Cantilever():
    ...
    def __init__(self, param, comm = None):
        ...
        self.baseAngles = param[11]
        

        self.theta = np.asarray([np.pi/4, -1000.0, 0.0, -1000.0, 3.*np.pi/4])
    ...
    def getDataDof(self):
        ...
        seed(12) # So the data points are the same each time
    ...
    def setTheta(self, perturbed_angles):
        for i in range(len(self.baseAngles)):
            self.theta[2 * i] = self.baseAngles[i] + perturbed_angles[i]
    ...
    def solve(self, dtheta, plotSolution = False, filename = "solution", iterativeSolver = True):

        self.setTheta(dtheta)
        ...
        for ie, e in enumerate(elem,0): # Loop over all local elements
            ...
            if(self.theta[layerId] >= -100.00): # Composite - theta is greater than 0
                C = self.composite
                isComposite = True
